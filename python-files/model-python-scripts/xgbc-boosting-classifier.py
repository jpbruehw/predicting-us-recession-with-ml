...
# make a list of the resulting models
...
path = '~/desktop/master-thesis-code/model-summaries/summary-xgbc-models.xlsx'
...
path_prob_raw = '../raw-pred/xgbc-pred-raw.xlsx'
...
for lag, res in zip(lags, gradient_boosting_results):
    # get the results
    res_data = res[1]
    # concat into new df
    res_df = pd.DataFrame({'y_true': res_data['y_true'], 'y_pred_raw': res_data['y_pred_raw']})
    # export to new sheet
    res_df.to_excel(writer_raw_prob, sheet_name=f"{lag}_xgbc", index=False)
# close writer

...
def run_svc_model(data, lag, test_size, scoring, params):
    ...
    roc_curve_plot(y_test, y_pred_raw, lag, roc_fig_dir_path, 'svc')
    
    # perform shap analysis on the model
    shap_analysis(best_model, X_train_resampled, X_test, shap_fig_dir_path, 'svc', lag)
...
svc_results = [(f"{lag}_month_lag_results", run_svc_model(data_raw, lag, 0.2, 'f1', param_grid)) for lag in lags]

# write data to excel to transfer to local file
# we will do further data processing in another script
path = '~/desktop/master-thesis-code/model-summaries/summary-svc-models.xlsx'
...
path_prob_raw = '../raw-pred/svc-pred-raw.xlsx'
...
for lag, res in zip(lags, svc_results):
    # get the results
    res_data = res[1]
    # concat into new df
    res_df = pd.DataFrame({'y_true': res_data['y_true'], 'y_pred_raw': res_data['y_pred_raw']})
    # export to new sheet
    res_df.to_excel(writer_raw_prob, sheet_name=f"{lag}_svc", index=False)
# close writer
